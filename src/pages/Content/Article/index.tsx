import { PlusOutlined, DeleteOutlined, CloseOutlined, RollbackOutlined, BaiduOutlined } from '@ant-design/icons';
...
import { article, addArticle, updateArticle, removeArticle, restoreArticle, delArticle, baiduArticle } from './service';
 ...
 * @param fields
 */
const handleUpdate = async (fields: EditModal, currentRow?: ArticleListItem) => {
...
const handleRemove = async (selectedRows: ArticleListItem[]) => {
...
const handleRestore = async (currentRow?: ArticleListItem) => {
...
const handleDelete = async (currentRow?: ArticleListItem) => {
...
const handleBaidu = async (selectedRows: ArticleListItem[]) => {
  const hide = message.loading('正在删除');
  if (!selectedRows) return true;
  try {
    await baiduArticle({
...
const ArticleList: React.FC = () => {
  ...
  const [currentRow, setCurrentRow] = useState<Partial<ArticleListItem> | undefined>(undefined);
  const [selectedRowsState, setSelectedRows] = useState<ArticleListItem[]>([]);
  ...
  return (
    <PageContainer>
      <ProTable<ArticleItem, API.PageParams>
        headerTitle={params ? <Button
            ...
            <Button
              ...
              onClick={() => {
                handleBaidu(selectedRowsState);
                setSelectedRows([]);
              }}
            >
              <BaiduOutlined /> 百度站长提交
            </Button>
      ...
      <EditModal
        ...
        onSubmit={async (value) => {
          ...
          if (method === 'add') {
            const success = await handleAdd(value as ArticleListItem);
            if (success) {
              handleEditModalVisible(false);
              if (actionRef.current) {
                actionRef.current.reload();
